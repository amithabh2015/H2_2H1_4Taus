...
//PU reweighting files 
  ...
  // standard
  float dzCutLoose = 0.1;
  float d0CutLoose = 0.1;
  // loose
  ...
  // decayMode = 0 - all hadronic decays
  // decayMode = 1 - 1-prong all
  // decayMode = 2 - 1-prong + 0 pi0
  // decayMode = 3 - 1-prong + 1 pi0
  // decayMode = 4 - 1-prong + 2 pi0
  // decayMode = 5 - 3-prong all
  ...
  int nEtaTauBins = 5;
  float etaTauBins[6] = {0., 0.8, 1.2, 1.6, 2.1, 2.4};
  ...
  TH1F * counterEtaTauH    = new TH1F("counterEtaTauH","",nEtaTauBins,etaTauBins);
  TH1F * counterEtaTauIdH  = new TH1F("counterEtaTauIdH","",nEtaTauBins,etaTauBins);
  TH1F * counterEtaTauIsoH = new TH1F("counterEtaTauIsoH","",nEtaTauBins,etaTauBins);
  TH1F * counterEtaTauNonLepH = new TH1F("counterEtaTauNonLepH","",nEtaTauBins,etaTauBins);
  
  TH1F * ptRecoTrackGenTrackH = new TH1F("ptRecoTrackGenTrackH","",400,-2,2);
  TH1F * ptRecoTrackGenTauH   = new TH1F("ptRecoTrackGenTauH","",800,-6,2);
  ...
  TH1F * HardMuonPtH = new TH1F("HardMuonPtH","",100,0,200.0);
  TH1F * SoftMuonPtH = new TH1F("SoftMuonPtH","",100,0,200.0);
  TH1F * SumOfInvMass2SystemsH=new TH1F("SumOfInvMass2SystemsH","",50,0.,25.);
  TH1F * InvMassH2H= new TH1F("InvMassH2H","",200,0.,200.);
  TH1F * PtTrackHardH=new TH1F("PtTrackHardH","",100,0.,100.);
  TH1F * PtTrackSoftH=new TH1F("PtTrackSoftH","",100,0.,100.);
  ...
  // Muon selection
  TH1F * HardMuonPt_DimuonsH = new TH1F("HardMuonPt_DimuonsH","",100,0.,200.);
  TH1F * SoftMuonPt_DimuonsH = new TH1F("SoftMuonPt_DimuonsH","",100,0.,200.);
...
// 	std::cout << std::endl;
	...
	for (int iP=0; iP<NPartFirstH1; ++iP) {
      ...
      bool decayModePassedFirst  = !firstTrackNotFound;
      bool decayModePassedSecond = !secondTrackNotFound;

      if (decayMode==1) { // all hadronic 1-prong
	decayModePassedFirst = decayModePassedFirst && (nChargedFirst==2);
	decayModePassedSecond = decayModePassedSecond && (nChargedSecond==2);
      }
      if (decayMode==2) { // 1-prong + 0 neutrals
	decayModePassedFirst = decayModePassedFirst && 
	  (nChargedFirst==2) && (nPhotonsFirst==0) && (nK0First==0);
	decayModePassedSecond = decayModePassedSecond && 
	  (nChargedSecond==2) && (nPhotonsSecond==0) && (nK0Second==0);
	
      }
      if (decayMode==3) { // 1-prong + 1 pi0
	decayModePassedFirst = decayModePassedFirst && 
	  (nChargedFirst==2) && (nPhotonsFirst==2) && (nK0First==0);
	decayModePassedSecond = decayModePassedSecond && 
	  (nChargedSecond==2) && (nPhotonsSecond==2) && (nK0Second==0);
	
      }
      if (decayMode==4) { // 1-prong + 2 pi0
	decayModePassedFirst = decayModePassedFirst && 
	  (nChargedFirst==2) && (nPhotonsFirst==4) && (nK0First==0);
	decayModePassedSecond = decayModePassedSecond && 
	  (nChargedSecond==2) && (nPhotonsSecond==4) && (nK0Second==0);
	
      }
      if (decayMode==5) { // all 3-prong 
	decayModePassedFirst = decayModePassedFirst && (nChargedFirst==4);
	decayModePassedSecond = decayModePassedSecond && (nChargedSecond==4);
      }
...
//	      if (ChargepfNoPU[itrack]*_posLepQ<0) {
	...
	if ( (PosFirst && !firstMuonNotFound && decayModePassedFirst) || 
	     (PosSecond && !secondMuonNotFound && decayModePassedSecond) ) {
	  ...
	  if (etaTau<2.4 && ptTau>0.5) {
	    ...
	    counterEtaTauH->Fill(etaTau,weight);
	    ...
	    TLorentzVector Track4; Track4.SetXYZM(0,0,0,pionMass); 
	    
	    float dRMIN = 0.3;

	    for (int iPart=0; iPart<NpfNoPU; iPart++) {
	      TLorentzVector part4; part4.SetXYZM(PxpfNoPU[iPart],
						  PypfNoPU[iPart],
						  PzpfNoPU[iPart],
						  pionMass);
	      TLorentzVector diffPos = part4 - MuonPos4;
	      TLorentzVector diffNeg = part4 - MuonNeg4;
	      float resoDR = deltaR(part4.Eta(),part4.Phi(),
				    posTrkGen4.Eta(),posTrkGen4.Phi());
	      if (resoDR<dRMIN && diffPos.P()>0.1 && diffNeg.P()>0.1 && PtpfNoPU[iPart]>ptSoftTrkCut 
		  && TMath::Abs(ippfNoPU[iPart])<d0CutTracks && TMath::Abs(ipZpfNoPU[iPart])<dzCutTracks
		  && (ChargepfNoPU[iPart]>0.5 ||ChargepfNoPU[iPart]<-0.5)) {
		Track4     = part4;
		dRMIN      = resoDR;
		foundTrack = true;
	      }
		...
		TLorentzVector diffPos4 = part4 - MuonPos4;
		TLorentzVector diffNeg4 = part4 - MuonNeg4;
		TLorentzVector diffTrk4 = part4 - Track4;

		float dRConeTrackPart = deltaR(part4.Eta(),part4.Phi(),
					       Track4.Eta(),Track4.Phi());

		if (diffPos4.P()>0.1 && diffNeg4.P()>0.1 && diffTrk4.P()>0.1 && PtpfNoPU[iPart]>ptTrkCut 
		    && TMath::Abs(ippfNoPU[iPart])<d0CutLoose && TMath::Abs(ipZpfNoPU[iPart])<dzCutLoose
		    && (ChargepfNoPU[iPart]>0.5 ||ChargepfNoPU[iPart]<-0.5) && dRConeTrackPart<0.5) {
		  nIsoTracks++; 
		}
	      ...
	      counterEtaTauIdH->Fill(etaTau,weight);
		...
		counterEtaTauIsoH->Fill(etaTau,weight);

		float dPt = ( Track4.Pt() - posTrkGen4.Pt() ) / posTrkGen4.Pt();

		ptRecoTrackGenTrackH->Fill(dPt,weight);

		dPt = ( Track4.Pt() - ptTau ) / ptTau;

		ptRecoTrackGenTauH->Fill(dPt,weight);
		...
		if (idtrk!=11 && idtrk!=13) {
		  ...
		  counterEtaTauNonLepH->Fill(etaTau,weight);
        ...
        if ( (NegFirst && !firstMuonNotFound && decayModePassedFirst) 
	     || (NegSecond && !secondMuonNotFound && decayModePassedSecond) ) {
	  ...
	  if (etaTau<2.4&&ptTau>0.5) {
		...
		float dPt = ( Track4.Pt() - negTrkGen4.Pt() ) / negTrkGen4.Pt();
	...
	if (dRPosMuJetMin<0.5 && posMuJet4.Eta()>-2.4 && posMuJet4.Eta()<2.4) {
	  ...
	  if (NpfNoPuIpPos>0) {
	    ...
	    counterPtTauIdH->Fill(ptTau,weight);
            counterPtPosMuTrkTauIdH->Fill(ptTrk,ptTau,weight);
            counterPtTrkTauIdH->Fill(ptTrk,ptTau,weight);
	    TLorentzVector Track4; Track4.SetXYZM(PxPfNoPuIpPos[0],
						  PyPfNoPuIpPos[0],
						  PzPfNoPuIpPos[0],
						  pionMass);
	...
	if (dRNegMuJetMin<0.5 && negMuJet4.Eta()>-2.4 && negMuJet4.Eta()<2.4 ) {
          ...
          if (NpfNoPuIpNeg>0) {
            ...
            counterPtNegMuTrkTauIdH->Fill(ptTrk,ptTau,weight);
            counterPtTrkTauIdH->Fill(ptTrk,ptTau,weight);
            TLorentzVector Track4; Track4.SetXYZM(PxPfNoPuIpNeg[0],
                                                  PyPfNoPuIpNeg[0],
                                                  PzPfNoPuIpNeg[0],
                                                  pionMass);
