...
int main(int argc, char * argv[]) {
  ...
  float ptTrkLower = 2.5;
  float ptTrkUpper = 6.0;
  int NsoftTracks = 1;
  ...
  if (argc!=6) {
    std::cout << "Usage of the program : HTauTauAnalysisMacro [file_list] [dR(mu-mu)] [Ip scale up] [Ip scale down] [NSoftTracks]" << std::endl;
    ...
    std::cout << "dR(mu-mu) cut : 0.5, 1.0 1.5, 2.0 " << std::endl;
  ...
  std::string dRcut(argv[2]);
  std::string RelaxedIp(argv[3]);
  std::string IpDown(argv[4]);  
  std::string NSoftTracks(argv[5]);
  ...
  TString FullName = TStrName+"_dRcut"+dRcut+"_IpUp"+RelaxedIp+"_IpDown"+IpDown+"_NTrk"+NSoftTracks+"_BCR2";

  TFile * file = new TFile(FullName+TString(".root"),"recreate");

  file->cd("");
  EventWeight * eventWeight = new EventWeight("/afs/desy.de/user/r/rasp/public/Analysis");
