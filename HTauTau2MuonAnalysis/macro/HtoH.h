...
TH1F * TH1toTH1(TH1F * histo, int n1BinsX, float * xBins, bool setErr,TString suffix) {

  TString name = TString(histo->GetName()) + suffix;

  TH1F * newHisto = new TH1F(name,"",n1BinsX,xBins);

  int n1Bins = histo->GetNbinsX();
  
  for (int iB=0;iB<n1Bins;++iB) {
...
TH2F * TH2toTH2(TH2F * histo, int n1BinsX, float * xBins, int n1BinsY, float * yBins, TString suffix) {

  TString name = TString(histo->GetName()) + "_" + suffix;

  TH2F * newHisto = new TH2F(name,"",n1BinsX,xBins,n1BinsY,yBins);
